...
def getModel_UD(textList):
    ...
    BOW.addstoplist(stoplist)
    BOW.build_index(textList, max(min(len(textList)/50,50),1))
    pModel=BOW.est_probability_UD(textList)
    return pModel
"""
    pdb.set_trace();
...
def perplexity(pModel, summTextper):
    ...
    for i in range(len(summTextper)):
        ...
        for wordVocab in pModel.keys():
            ...
            if (TFVec[j]!=0):
                lkHood=lkHood+math.log(TFVec[j]*pModel[wordVocab])
            j=j+1

        likelihood[i]=math.exp(lkHood)
    perScore=sum(likelihood)
    return perScore;
